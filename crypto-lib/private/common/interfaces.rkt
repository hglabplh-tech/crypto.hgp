...
(define cms-sign<%>
  (interface (impl<%>)
    [cms-sign-sure            (->m bytes? bytes? bytes? list? bytes? integer?)]
    [cms-init-signing         (->m any/c  bytes? bytes? list? bytes? integer?)]
    ...
    [cms-add-receipient-cert  (->m any/c bytes? integer?)]
